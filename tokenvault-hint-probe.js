...
(async()=>{
 ...
 const withHint=await post(env.AUTH0_DOMAIN,'/oauth/token',{...base,login_hint: scope==='execute:refund'?env.CFO_TOKEN_VAULT_LOGIN_HINT:env.DPO_TOKEN_VAULT_LOGIN_HINT});
 const noHint=await post(env.AUTH0_DOMAIN,'/oauth/token',base);
 console.log('with_hint',withHint.status,withHint.body);
 console.log('no_hint',noHint.status,noHint.body);
})();
